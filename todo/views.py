...
class TodoView(APIView):
    """
    Todo View is used to handle the incoming requests relating to
    'todo' items
    """
    ...
    def get(self, request, pk=None):
        ...
        Gets 'username' from the 'query_param' in order to retrieve the
        'todo' items belonging to that user, then checks to see if a primary key
        has been provided by the URL. If not, a full list of 'todo' will be retrieved.
        If a primary key has been provided then only that instance will be retrieved.

        If no username is found in the 'query_params' then a 404 (not found)
        ...
        if "username" in request.query_params:
            ...
            if pk is not None:
                ...
                # Serialize the data from the DB and serialize them using 'TodoSerializer'
                serializer = TodoSerializer(todo)
                # Store the serialized data in 'serialized_data'
